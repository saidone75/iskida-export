...
(defn create-image [path gallery-id image-name]
  (let [image-map (parse-image-ffc image-name)]
    (spit "/home/saidone/Vagrant VMs/veramente-deb/copy-images.sh" (str "cp '" image-name "&main.jpg' " "joomgallery/details/" path "/" image-name ".jpg" "\n") :append true)
    (spit "/home/saidone/Vagrant VMs/veramente-deb/copy-images.sh" (str "mogrify -path joomgallery/thumbnails/" path " -thumbnail 160x160 joomgallery/details/" path "/" image-name ".jpg" "\n") :append true)
    (str "INSERT INTO `"
         config/table-prefix
         ...
         (get-image-id) ","
         gallery-id ","
         "'" (s/replace (s/replace (.toLowerCase (:title image-map)) #" " "_") #"'" "_") "'" ","
...
(defn create-gallery [sql gallery]
  (let [sql 
        (str sql
             "INSERT INTO `"
             config/table-prefix
             "joomgallery_catg` (`cid`, `name`, `alias`, `parent_id`, `lft`, `asset_id`, `level`, `published`, `catpath`, `access`, `params`, `metakey`, `metadesc`, `exclude_toplists`, `exclude_search`) VALUES ("
             (key gallery) ","
    ...
    (spit "/home/saidone/Vagrant VMs/veramente-deb/copy-images.sh" (str "mkdir -p joomgallery/details/" (s/replace (.toLowerCase (:title (val gallery))) #" " "_") "\n") :append true)
    (spit "/home/saidone/Vagrant VMs/veramente-deb/copy-images.sh" (str "mkdir -p joomgallery/thumbnails/" (s/replace (.toLowerCase (:title (val gallery))) #" " "_") "\n") :append true)
    (str sql
         (apply str
                (do
...
(spit "/home/saidone/Vagrant VMs/veramente-deb/galleries.sql"
      (let [galleries
            (reduce
             parse-gallery
             {}
             (take 2 config/galleries))]
        (reduce
         create-gallery
         ""
         galleries)))
